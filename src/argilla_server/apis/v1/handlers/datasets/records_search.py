...
from argilla_server import search_engine as search_engine
...
async def search_current_user_dataset_records(
    *,
    ...
    record_id_score_map: Dict[UUID, Any] = {
...
def _to_search_engine_filter(filters: Filters, user: Optional[User]) -> search_engine.Filter:
    ...
    for search_filter in filters.and_:
        engine_scope = _to_search_engine_filter_scope(search_filter.scope, user=user)

        if isinstance(search_filter, TermsFilter):
            engine_filter = search_engine.TermsFilter(scope=engine_scope, values=search_filter.values)
        elif isinstance(search_filter, RangeFilter):
            engine_filter = search_engine.RangeFilter(scope=engine_scope, ge=search_filter.ge, le=search_filter.le)
        else:
            raise Exception(f"Unknown filter type {type(search_filter)}")
